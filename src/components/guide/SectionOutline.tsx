    ...
    >
      <div className={`flex flex-nowrap sm:flex-wrap items-center gap-1.5 overflow-x-auto ${SCROLL_ROOM}`}>
        ...
        <span className="shrink-0 ml-auto flex items-center gap-2 pl-2 text-xs">
          <button type="button" onClick={onExpandAll} className={`${TAP} text-neutral-800 dark:text-[#d4d4d4] hover:underline font-semibold cursor-pointer`}>ขยายทั้งหมด</button>
          <span className="text-neutral-300 dark:text-[#333333]">|</span>
          <button type="button" onClick={onCollapseAll} className={`${TAP} text-neutral-500 dark:text-[#737373] hover:underline font-semibold cursor-pointer`}>ย่อทั้งหมด</button>
        </span>
      </div>
    </div>
  );
