...
- ให้คำแนะนำเชิงปฏิบัติการ (Actionable advice) เช่น รูปประโยคที่ควรพูดในที่ประชุม หรือขั้นตอนตกลงร่วมกัน
...
กรุณาให้คำตอบที่กระจ่าง ชัดเจน แบ่งข้อคิดและแนวทางแก้ไขที่ใช้งานได้จริงในองค์กรยุคใหม่:`;
      ...
      const answer = response.text || "ขออภัย ไม่สามารถสร้างคำตอบได้ กรุณาลองใหม่อีกครั้ง";
...
3. **คำแนะนำเชิงปฏิบัติ (สิ่งที่ควรพูดในห้องประชุม):**
   - ถามว่า: *"เวอร์ชันที่เร็วและเล็กที่สุด (MVP) ของปุ่มนี้ ตัดเงื่อนไขอะไรออกไปก่อนได้บ้างเพื่อลดเวลาส่งมอบ?"*`;
...
- **วิธีแก้:**
  ...
  3. เจรจาจัดสรรเวลาคงที่ เช่น 15-20% ของ Capacity ทุก Sprint สำหรับเสถียรภาพระบบ`;
      } else {
        fallbackAnswer = `**คำแนะนำจากผู้เชี่ยวชาญ:**

เรื่องนี้เป็นความท้าทายคลาสสิกที่เกิดจากความต่างของเป้าหมาย:
...
3. **ใช้ Visual Diagram ที่ถูกระดับ:** เช่น C4 Level 1 สำหรับคุยภาพรวมกับผู้บริหาร และ Sequence สำหรับคุยรายละเอียดขั้นตอนกับทีม`;
      ...
      return res.json({ 
        ...
        source: "fallback",
        note: "กำลังใช้งานระบบวิเคราะห์อัจฉริยะในเครื่อง (สามารถเพิ่ม GEMINI_API_KEY ใน Settings เพื่อเปิดใช้ Gemini AI แบบเต็มรูปแบบ)" 
      });
    }
  } catch (err: any) {
    res.status(500).json({ error: err.message || "เกิดข้อผิดพลาดในการประมวลผล" });
  }
});
